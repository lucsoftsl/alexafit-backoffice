...
const UserDetailModal = ({ isOpen, onClose, user, fromPage }) => {
  ...
  const loadNutritionData = useCallback(async () => {
    ...
    try {
      ...
      setError(t('Failed to load nutrition data'))
    } finally {
      setLoading(false)
    }
  ...
  const handleSendNotification = async () => {
    if (!user?.loginDetails?.pushNotificationToken) {
      alert(t('No push notification token available for this user'))
      return
    }

    if (!notificationMessage.trim()) {
      alert(t('Please enter a notification message'))
      return
    }

    if (!nutritionData) {
      alert(t('No nutrition data available'))
      return
    }
    ...
    try {
      ...
      alert(t('Failed to send notification. Please try again.'))
    } finally {
  ...
  const handleSendPushNotification = async () => {
    ...
    if (!notificationTitle.trim() || !notificationBody.trim()) {
      alert(t('Please enter both notification title and body'))
      return
    }
    ...
    try {
      ...
      alert(t('Push notification sent successfully!'))
      setShowPushNotificationPopup(false)
      setNotificationTitle('You received a new message')
      setNotificationBody('')
    } catch (err) {
      alert(t('Failed to send push notification. Please try again.'))
    } finally {
  ...
  const handleUpdateMessage = async (messageId) => {
    ...
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, 1500))
      await refreshMessages() // Refresh messages (clear cache and reload)
      alert(t('Message updated successfully!'))
    } catch (err) {
      alert(t('Failed to update message. Please try again.'))
    }
  }
  ...
  const handleDeleteMessage = async (messageId) => {
    ...
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, 1500))
      await refreshMessages() // Refresh messages (clear cache and reload)
      alert(t('Message deleted successfully!'))
    } catch (err) {
      alert(t('Failed to delete message. Please try again.'))
    }
  }
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            ...
            {fromPage === 'unapprovedItems' && <>
              ...
              <p className="text-gray-600 mt-1">{t('common.Name')}: {user?.loginDetails?.displayName || user?.userData?.name}</p>
              <p className="text-gray-600 mt-1">{t('common.Email')}: {user?.loginDetails?.providerData[0]?.email || user?.loginDetails?.email || user?.userData?.email}</p>
              <p className="text-gray-600 mt-1">{t('common.ProviderId')}: {user?.loginDetails?.providerData[0]?.providerId || user?.loginDetails?.providerId}</p>
            </>}
          </div>
          <button
            onClick={() => {
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
          ...
          {loading && (
            <div className="flex items-center justify-center py-12">
              ...
              <span className="ml-4 text-gray-600">{t('common.Loading nutrition data...')}</span>
            </div>
          )}
              ...
              <div className="space-y-6">
                ...
                <div className="card p-6">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                    ...
                    {t('common.Daily Summary (Actual Consumed)')}
                  </h3>
                  <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-4">
                    <div className="text-center">
                      ...
                      <div className="text-sm text-gray-600">{t('common.Total Calories')}</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-green-600">
                        {formatNutritionValue(actualTotals.totalProtein)}g
                      </div>
                      <div className="text-sm text-gray-600">{t('common.Protein')}</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-yellow-600">
                        {formatNutritionValue(actualTotals.totalCarbs)}g
                      </div>
                      <div className="text-sm text-gray-600">{t('common.Carbs')}</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-purple-600">
                        {formatNutritionValue(actualTotals.totalFat)}g
                      </div>
                      <div className="text-sm text-gray-600">{t('common.Fat')}</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-red-600">
                        {formatNutritionValue(nutritionData.totalCalories || 0)}
                      </div>
                      <div className="text-sm text-gray-600">{t('common.API Total Calories')}</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-emerald-600">
                        {dailyScore}%
                      </div>
                      <div className="text-sm text-gray-600">{t('common.Daily Score')}</div>
                    </div>
                  </div>
                  {/* Completion badges */}
                  ...
                  {user?.loginDetails?.pushNotificationToken && (
                    <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                      <h4 className="text-sm font-medium text-blue-900 mb-2 flex items-center">
                        ...
                        {t('common.Send Daily Score Notification')}
                      </h4>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="text-center">
                      ...
                      <div className="text-sm text-gray-600">{t('common.Fiber')}</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-semibold text-pink-600">
                        {formatNutritionValue(actualTotals.totalSugar)}g
                      </div>
                      <div className="text-sm text-gray-600">{t('common.Sugar')}</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-semibold text-red-600">
                        {formatNutritionValue(nutritionData.exerciseCalories || 0)}
                      </div>
                      <div className="text-sm text-gray-600">{t('common.Exercise Calories')}</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-semibold text-indigo-600">
                        {formatNutritionValue(nutritionData.percentageOfGoal || 0)}%
                      </div>
                      <div className="text-sm text-gray-600">{t('common.Goal Progress')}</div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="card p-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div className="text-center">
                        ...
                        <div className="text-sm text-gray-600">{t('common.BMI')}</div>
                      </div>
                      <div className="text-center">
                        <div className="text-lg font-semibold text-green-600">
                          {formatNutritionValue(nutritionData.goalNutrients.BMR)}
                        </div>
                        <div className="text-sm text-gray-600">{t('common.BMR (Calories)')}</div>
                      </div>
                      <div className="text-center">
                        <div className="text-lg font-semibold text-yellow-600">
                          {formatNutritionValue(nutritionData.goalNutrients.TDEE)}
                        </div>
                        <div className="text-sm text-gray-600">{t('common.TDEE (Calories)')}</div>
                      </div>
                      <div className="text-center">
                        <div className="text-lg font-semibold text-purple-600">
                          {nutritionData.goalNutrients.foodTables || 'N/A'}
                        </div>
                        <div className="text-sm text-gray-600">{t('common.Food Tables')}</div>
                      </div>
                    </div>
                  </div>
                )}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {Object.entries(nutritionData.totalCaloriesPerMeal).map(([meal, calories]) => (
                        <div key={meal} className="text-center">
                          ...
                          <div className="text-sm text-gray-600 capitalize">{t(`common.${meal}`)}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                    ...
                    <div className="mt-4 space-y-4">
                      ...
                      {loadingMessages ? (
                        <div className="flex items-center justify-center py-8">
                          ...
                          <span className="ml-3 text-gray-600">{t('common.Loading messages...')}</span>
                        </div>
                      ) : messageError ? (
                        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
...
{t('common.Actions')}
                                ...
                                return (
                                  <div className="flex items-center justify-between mt-4">
                                    <div className="text-sm text-gray-700">
                                      {t('common.Showing')} {((currentPage - 1) * itemsPerPage) + 1} {t('common.to')} {Math.min(currentPage * itemsPerPage, messages.length)} {t('common.of')} {messages.length} {t('common.messages')}
                                    </div>
                                    <div className="flex gap-2">
                                      <button
                ...
                {(() => {
                  ...
                  return (
                    <div className="card p-6">
                      <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('common.Shareable Summary')}</h3>
                ...
                {!nutritionData.userItems && !nutritionData.totalCalories && (
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">{t('common.No Nutrition Data')}</h3>
      ...
      {showPushNotificationPopup && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            {/* Header */}
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h3 className="text-xl font-bold text-gray-900">{t('common.Send Push Notification')}</h3>
              <button
                onClick={() => {
                  setShowPushNotificationPopup(false)
                  setNotificationTitle('You received a new message')
