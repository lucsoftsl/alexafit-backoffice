...
const glassCardClass = 'relative overflow-hidden rounded-3xl border border-white/40 bg-white/70 backdrop-blur-xl shadow-[0_20px_80px_rgba(15,23,42,0.08)]'
const glassSurfaceClass = 'rounded-2xl border border-white/50 bg-white/60 backdrop-blur-lg shadow-[0_10px_40px_rgba(15,23,42,0.08)]'
                  ...
                  <>
                    <div className="space-y-4">
                      {paginatedTemplates.map(template => (
                        <div key={template.id} className={`${glassSurfaceClass} relative p-5`}> 
                          ...
                          <div className="relative space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 text-xs">
                              {mealTypeOptions.map(meal => (
                                <div key={meal.id} className="rounded-xl border border-white/60 bg-white/80 px-3 py-2 text-center font-semibold text-gray-800 shadow-inner">
                                  <p className="text-[11px] uppercase tracking-wide text-gray-500">{meal.label}</p>
                                  <p className="text-sm text-gray-900">{template[meal.id]?.length || 0} items</p>
                                </div>
                              ))}
                            </div>
                              ...
                              {clients.length === 0 ? (
                                ...
                                <div className="space-y-1 max-h-44 overflow-y-auto">
                                  {clients.map(client => {
                                    ...
                                    return (
                                      <div key={clientId} className="rounded-lg border border-white/70 bg-white/90 px-3 py-2">
                                        <button
                  ...
                  {searchResults.length > 0 && (
                    <div className="max-h-64 overflow-y-auto rounded-2xl border border-white/70 bg-white/80 backdrop-blur divide-y divide-white/70">
                      {searchResults.map((item, idx) => {
                        ...
                        return (
                          <div key={idx} className="flex items-center justify-between px-4 py-3 transition hover:bg-white">
                            <div className="min-w-0">
                              <div className="text-sm font-semibold text-gray-900 truncate">{item?.name || item?.title || 'Unnamed'}</div>
                  ...
                  {plans[activeMealType]?.length > 0 && (
                    <div className="space-y-3 rounded-2xl border border-white/70 bg-white/80 p-4 backdrop-blur">
                      ...
                      {plans[activeMealType].map((item, idx) => {
                        ...
                        return (
                          <div key={idx} className="relative rounded-2xl border border-white/60 bg-white/90 p-4 shadow-sm">
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                <div className="text-sm font-semibold text-gray-900 truncate">{item?.name || item?.title || 'Unnamed'}</div>
                                <div className="text-xs text-gray-600 truncate">{isRecipeItem ? 'Recipe' : 'Food'}</div>
                              </div>
                              <button
                                onClick={() => removeItemFromPlan(activeMealType, idx)}
                                  ...
                                  <div className="flex items-center gap-2">
                                    <input
                                      ...
                                      onBlur={(e) => {
                                        const inputValue = e.target.value
                                        if (inputValue === '') {
                                          const originalServingAmountValue = item?.originalServingAmount || 100
                                          setDisplayValues(prev => ({
                                            ...prev,
                                            [itemKey]: {
                                              selectedServingId: currentServingId,
                                              servingAmount: originalServingAmountValue,
                                            }
                                          }))
                                        }
                                      }}
