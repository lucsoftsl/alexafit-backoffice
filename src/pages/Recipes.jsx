...
const Recipes = () => {
    ...
    const numberOfPages = Math.ceil(totalItemCount / pageSize)

    useEffect(() => {
        // Reset and load first page when component mounts or country code changes
        setRecipeItems([])
        setCurrentPage(0)
        setHasMore(true)
        loadPage(0)
    ...
    // Load page when currentPage changes (but not on initial mount)
    useEffect(() => {
        // Skip if this is the initial load (handled by filterCountryCode effect)
        const isInitialLoad = currentPage === 0 && recipeItems.length === 0
        if (!isInitialLoad) {
    ...
    const loadPage = async (page) => {
        try {
            ...
            const data = await getRecipesByCountryCode({ countryCode: filterCountryCode, page })

            const newItems = data.items || []
            setRecipeItems(newItems)
            setTotalItemCount(data.total || 0)
            setPageSize(data.pageSize || 25)
            setLoading(false)

            // If we got less than 25 items, there are no more pages
            ...
            console.log(`Loaded page ${page} from API`)
        } catch (err) {
    ...
    const handleJumpToPage = () => {
        ...
        if (pageNumber >= 1 && pageNumber <= numberOfPages) {
            setCurrentPage(pageNumber - 1) // Convert to 0-indexed
            setPageJumpValue('')
        } else {
            alert(`Please enter a page number between 1 and ${numberOfPages}`)
        }
    }
    ...
    const updateIngredientWeight = (index, weight) => {
        ...
        updated[index].servingDisplay = `${weight} ${updated[index].unit}`
    ...
    const updateIngredientUnit = (index, unit) => {
        ...
        updated[index].servingDisplay = `${updated[index].quantity} ${unit}`
        setSelectedIngredients(updated)
    }
    ...
    const calculateNutrients = (ingredients, updateWeightAfterCooking = true) => {
        const totalNutrs = ingredients.reduce(
            (acc, ingredient) => {
                const amount = parseFloat(ingredient.weight) || 0
                const totalCalories = acc.totalCalories + ((ingredient?.totalCalories || 0) * amount) / 100
                const totalQuantity = acc.totalQuantity + parseInt(amount, 10)

                const totalProtein =
                    acc.totalProtein +
                    (amount * (ingredient?.totalNutrients?.proteinsInGrams || 0)) / 100
                const totalCarbs =
                    acc.totalCarbs +
                    (amount * (ingredient?.totalNutrients?.carbohydratesInGrams || 0)) / 100
                const totalFat =
                    acc.totalFat + (amount * (ingredient?.totalNutrients?.fatInGrams || 0)) / 100
                const totalFiber =
                    acc.totalFiber +
                    (amount * (ingredient?.totalNutrients?.fibreInGrams || 0)) / 100
                const totalSaturatedFat =
                    acc.totalSaturatedFat +
                    (amount * (ingredient?.totalNutrients?.fattyAcidsTotalSaturatedInGrams || 0)) / 100
                const totalUnSaturatedFat =
                    acc.totalUnSaturatedFat +
                    (amount * (ingredient?.totalNutrients?.fattyAcidsTotalUnSaturatedInGrams || 0)) / 100
                const totalSugar =
                    acc.totalSugar +
                    (amount * (ingredient?.totalNutrients?.sugarsInGrams || 0)) / 100

                const totalSalt =
                    acc.totalSalt +
                    (amount * (ingredient?.totalNutrients?.saltInGrams || 0)) / 100

                const totalCholesterol =
                    acc.totalCholesterol + (amount * (ingredient.cholesterol || 0)) / 100

                const totalPotassium =
                    acc.totalPotassium + (amount * (ingredient.potassium || 0)) / 100
    ...
    // Generate default serving options
    const generateDefaultServings = (countryCode) => {
        // Default serving options based on country code
        const servings = [
            {
                amount: 100,
                unit: 'grams',
        ...
        if (countryCode === 'hu') {
            ...
            // Generic servings for other countries
            servings.push({
                amount: 100,
                unit: 'grams',
                innerName: 'Portion',
    ...
    const handleCreateRecipe = async () => {
        ...
        try {
            ...
            const serving = generateDefaultServings(selectedCountryCode)
                ...
                <div className="card p-6">
                    <div className="flex items-center">
                        ...
                        <div className="ml-4">
                            ...
                            <p className="text-2xl font-semibold text-gray-900">{totalItemCount}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="card overflow-hidden">
                ...
                <div className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200">
                    <div className="text-sm text-gray-700">
                        Showing {filteredRecipes.length} recipe{filteredRecipes.length !== 1 ? 's' : ''} on page {currentPage + 1} of {numberOfPages}
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            <button
                                ...
                                disabled={loading || !pageJumpValue}
